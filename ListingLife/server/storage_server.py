...
def initialize_storage():
    ...
    if config:
        ...
        LOCAL_STORAGE_PATH = Path(config.get('local_storage_path', '../data'))
                ...
                try:
                    ...
                    while retry_count < max_retries and not connection_success:
                        try:
                            ...
                            # Test BOTH scopes by doing a test upload/download
                            test_file_path = f"{DROPBOX_FOLDER.rstrip('/')}/.scope_test"
                            try:
                                # Test write scope
                                dropbox_client.files_upload(
                                    b'scope_test',
                                    test_file_path,
                                    mode=dropbox_imported.files.WriteMode('overwrite')
                                )
                                # Test read scope
                                _, response = dropbox_client.files_download(test_file_path)
                                # Clean up
                                try:
                                    dropbox_client.files_delete(test_file_path)
                                except:
                                    pass
                                logger.info("✅ Dropbox scopes verified (read and write)")
                            except dropbox_imported.exceptions.ApiError as scope_error:
                                error_msg = str(scope_error)
                                if 'files.content.read' in error_msg or 'files.content.write' in error_msg or 'required scope' in error_msg.lower():
                                    logger.error(f"❌ Dropbox scope error: {error_msg}")
                                    logger.error("   Your token is missing required scopes. Go to https://www.dropbox.com/developers/apps")
                                    logger.error("   Select your app -> Permissions tab -> Enable BOTH 'files.content.read' AND 'files.content.write'")
                                    logger.error("   Then regenerate your access token and update it in settings.")
                                    raise Exception(f"Dropbox scope error: {error_msg}")
                                else:
                                    # Not a scope error, might be folder doesn't exist, etc. - continue
                                    logger.warning(f"⚠️  Scope test warning (non-critical): {error_msg}")
...
def save_to_dropbox(key, data):
    ...
    try:
        ...
        elif 'files.content.write' in error_msg or 'files.content.read' in error_msg or 'required scope' in error_msg.lower():
            scope_needed = 'files.content.read' if 'files.content.read' in error_msg else 'files.content.write'
            raise Exception(
                f"Dropbox permission error: Your app needs BOTH 'files.content.read' AND 'files.content.write' scopes.\n\n"
                f"The error indicates '{scope_needed}' is missing, but you need BOTH scopes enabled.\n\n"
                "To fix:\n"
                "1. Go to https://www.dropbox.com/developers/apps\n"
                "2. Select your app (click on it)\n"
                "3. Go to the 'Permissions' tab\n"
                "4. Under 'Files and folders', enable BOTH:\n"
                "   - files.content.read (required to load data)\n"
                "   - files.content.write (required to save data)\n"
                "5. Go back to 'Settings' → 'OAuth 2'\n"
                "6. Click 'Generate' to create a new access token\n"
                "7. Copy the new token and update it in your app settings\n"
...
def load_from_dropbox(key):
    ...
    try:
        ...
        # Check for permission/scope errors
        error_msg = str(e)
        if 'files.content.read' in error_msg or ('required scope' in error_msg.lower() and 'files.content' in error_msg):
            logger.error(f"Error loading from Dropbox {key}: Missing required scope 'files.content.read'")
            logger.error("   Your Dropbox app needs BOTH 'files.content.read' AND 'files.content.write' scopes.")
            logger.error("   Go to https://www.dropbox.com/developers/apps, select your app,")
            logger.error("   go to the 'Permissions' tab, enable BOTH scopes, then regenerate your access token.")
            # Don't raise - return None so app can continue with local data
            return None
    ...
    try:
        ...
        for key, value in items.items():
            try:
                ...
                if 'files.content.write' in error_msg or 'files.content.read' in error_msg or 'required scope' in error_msg.lower():
                    logger.error(f"Error syncing {key}: Dropbox permission error - BOTH 'files.content.read' AND 'files.content.write' scopes required")
                    logger.error("   Go to https://www.dropbox.com/developers/apps, select your app, go to 'Permissions' tab, enable BOTH scopes, regenerate token")
                else:
...
def test_storage():
    ...
    try:
        ...
        if mode == 'local':
            path = config.get('local_storage_path', '../data')
            test_path = Path(path)
            try:
            ...
            try:
                ...
                test_file_path = f"{config.get('dropbox_folder', '/ListingLife').rstrip('/')}/.test_scope_check"
                ...
                try:
                    ...
                    if 'files.content.write' in error_msg or 'files.content.read' in error_msg or 'required scope' in error_msg.lower():
                        return jsonify({
                            'error': "Dropbox permission error: Your app needs BOTH 'files.content.read' AND 'files.content.write' scopes. Go to https://www.dropbox.com/developers/apps, select your app, go to the 'Permissions' tab, enable BOTH scopes, then regenerate your access token."
                        }), 400
                    else:
                        return jsonify({'error': f'Dropbox write test failed: {error_msg}'}), 400
                
                # Test read permission by attempting to download the test file
                try:
                    _, response = test_client.files_download(test_file_path)
                    if response.content != b'test_data':
                        return jsonify({'error': 'Dropbox read test failed: Downloaded data does not match'}), 400
                    ...
                    try:
                        ...
                        pass
                    if 'files.content.read' in error_msg or 'required scope' in error_msg.lower():
                        return jsonify({
                            'error': "Dropbox permission error: Your app needs BOTH 'files.content.read' AND 'files.content.write' scopes. The 'files.content.read' scope is missing. Go to https://www.dropbox.com/developers/apps, select your app, go to the 'Permissions' tab, enable BOTH scopes, then regenerate your access token."
                        }), 400
                    else:
                        return jsonify({'error': f'Dropbox read test failed: {error_msg}'}), 400
