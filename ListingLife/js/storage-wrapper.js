    ...
    async syncToBackend(forceOverride = false) {
        ...
        try {
            ...
            if (Object.keys(items).length > 0) {
                // REMOVED: Aggressive backend checking that was causing multiple API errors
                // Just sync all items - the backend will handle duplicates appropriately
                // This is much simpler and doesn't cause constant API calls and errors
                ...
                if (Object.keys(itemsToSync).length > 0) {
                    if (forceOverride) {
                        ...
                        console.log(`🔄 Syncing ${Object.keys(itemsToSync).length} items to backend...`);
                    }
                ...
                const healthCheck = async () => {
                    try {
                        ...
                        if (response.ok) {
                            ...
                            // If storage mode changed, just update tracking
                            // REMOVED: Automatic page reload - this was causing constant refreshes during editing
                            // Users can manually reload if they need fresh data after fixing Dropbox connection
                            if (lastStorageMode !== currentMode) {
                                this.backendStorageMode = currentMode;
                                console.log(`📦 Storage mode changed: ${lastStorageMode} → ${currentMode}`);
                                
                                // Only dispatch event, don't reload page
