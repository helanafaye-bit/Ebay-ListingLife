...
class SoldItemsTrends {
    constructor() {
        ...
        this.trendingViewSelect = document.getElementById('trendingViewSelect');
        this.trendingSubtitle = document.getElementById('trendingSubtitle');
        this.currentTrendingView = 'keywords'; // 'keywords' or 'subcategories'
        this.init();
    }
    ...
    init() {
        ...
        if (!window.storeManager) {
            ...
            setTimeout(async () => {
                ...
                if (this.currentTrendingView === 'keywords') {
    ...
    setupEventListeners() {
        ...
        if (this.trendingViewSelect) {
            this.trendingViewSelect.addEventListener('change', (e) => {
                this.currentTrendingView = e.target.value;
                if (this.currentTrendingView === 'keywords') {
                    this.renderTrendingKeywords();
                } else {
                    this.renderTrendingSubcategories();
    ...
    addSubcategoryItemEditorRow(item = {}) {
        ...
        const photoHtml = photoUrl 
            ? `<div class="subcategory-item-photo"><img src="${this.escapeHtml(photoUrl)}" alt="${safeLabel || 'Item photo'}" onerror="this.style.display='none'; const placeholder = this.parentElement?.querySelector('.subcategory-item-photo-placeholder'); if (placeholder) placeholder.style.display='flex';"></div><div class="subcategory-item-photo-placeholder" style="display: none;"><span>📷</span></div>`
            : `<div class="subcategory-item-photo" style="display: none;"><img src="" alt="${safeLabel || 'Item photo'}" onerror="this.style.display='none'; const placeholder = this.parentElement?.querySelector('.subcategory-item-photo-placeholder'); if (placeholder) placeholder.style.display='flex';"></div><div class="subcategory-item-photo-placeholder"><span>📷</span></div>`;
    ...
    renderCategories() {
        ...
        // Update trending view after rendering categories
        if (this.currentTrendingView === 'keywords') {
            this.renderTrendingKeywords();
        } else {
            this.renderTrendingSubcategories();
    ...
    renderTrendingKeywords() {
        ...
        // Update subtitle
        if (this.trendingSubtitle) {
            this.trendingSubtitle.textContent = 'Most sold items by keyword';
        }

        // Update header title
        const headerTitle = document.querySelector('.trending-keywords-header h2');
        if (headerTitle) {
            headerTitle.textContent = 'Trending Keywords';
        }
    ...
    renderTrendingSubcategories() {
        if (!this.trendingKeywordsList) return;

        // Update subtitle
        if (this.trendingSubtitle) {
            this.trendingSubtitle.textContent = 'Most sold items by subcategory';
        }

        // Update header title
        const headerTitle = document.querySelector('.trending-keywords-header h2');
        if (headerTitle) {
            headerTitle.textContent = 'Trending Subcategories';
        }
        ...
        // Collect all subcategories with their sold item counts
        ...
        if (subcategoryCounts.length === 0) {
            this.trendingKeywordsList.innerHTML = '<p style="text-align: center; color: #6b7280; padding: 20px;">No subcategories found</p>';
            return;
        }

        // Sort by count (descending) - most sold items at the top
        const sortedSubcategories = subcategoryCounts.sort((a, b) => b.count - a.count);

        let html = '';
        sortedSubcategories.forEach(({ name, count, categoryName }) => {
            html += `
                <div class="trending-keyword-item" data-subcategory-name="${this.escapeHtml(name)}">
                    <span class="trending-keyword-name">${this.escapeHtml(name)}</span>
                    <span class="trending-keyword-count">${count}</span>
                </div>
            `;
        });
        ...
        // Add click handlers to search for the subcategory
        this.trendingKeywordsList.querySelectorAll('.trending-keyword-item').forEach(item => {
            item.addEventListener('click', () => {
                const subcategoryName = item.dataset.subcategoryName;
                if (this.soldSearchBar) {
                    this.soldSearchBar.value = subcategoryName;
                    this.searchSoldItems();
                }
            });
        });
