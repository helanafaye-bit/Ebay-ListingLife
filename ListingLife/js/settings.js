class ListingLifeSettingsPage {
    ...
    async saveStorageSettings() {
        ...
        try {
            console.log('Saving storage config to server...', config);
            const response = await fetch('http://127.0.0.1:5000/api/storage/config', {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                ...
                if (result.success && result.message) {
                    this.showStorageStatus(result.message, 'success');
                } else {
                    this.showStorageStatus('Storage settings saved and server updated!', 'success');
                }
            } else {
                let error;
                try {
                ...
                if (error.message) {
                    this.showStorageStatus(error.message, 'error');
                } else {
                    this.showStorageStatus('Settings saved locally. Backend update failed: ' + (error.error || 'Unknown error'), 'warning');
                }
            }
        } catch (error) {
            console.error('Error saving to server:', error);
            this.showStorageStatus('Settings saved locally. Backend not available - please restart the storage server manually with the new settings. Error: ' + error.message, 'warning');
        }
    }
