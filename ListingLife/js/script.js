...
class EbayListingLife {
    ...
    handleCategoriesSort(e) {
        // Invalidate cache when sort order changes (since hash doesn't include sort order)
        this._invalidateCategoryCache();
    ...
    async loadData() {
                    ...
                    if (result.value) {
                        ...
                        try {
                            ...
                            if (backendData.categories !== undefined || backendData.items !== undefined) {
                                ...
                                // NOTE: We DON'T update localStorage here - we'll do it AFTER merging to preserve sold/ended status
                            }
                        } catch (parseError) {
        ...
        if (savedData) {
            try {
                ...
                // If we loaded from backend, ALWAYS save merged data to localStorage
                // This ensures backend data is cached, and sold/ended status is preserved
                if (loadedFromBackend) {
                    const mergedCount = localSoldEndedMap.size;
                    if (mergedCount > 0) {
                        console.log(`   Merged ${mergedCount} sold/ended items from local storage`);
                    }
                    ...
                    try {
                        ...
                        console.log('✓ Saved merged data to localStorage' + (mergedCount > 0 ? ' (preserved sold/ended flags)' : ''));
                        // Note: localStorage.setItem will automatically queue backend save via Storage override
                        // This is async and non-blocking, so no refresh will occur
                    } catch (saveError) {
