...
def save_to_dropbox(key, data):
    ...
    try:
        ...
        if 'expired_access_token' in error_msg or 'expired' in error_msg.lower():
            raise Exception(
                "Your Dropbox access token has expired. Please generate a new token:\n\n"
                "1. Go to https://www.dropbox.com/developers/apps\n"
                "2. Select your app\n"
                "3. Go to Settings -> OAuth 2\n"
                "4. Click 'Generate access token'\n"
                "5. Copy the new token and update it in your settings"
            )
    ...
    try:
        ...
        if mode == 'local':
            ...
            try:
                ...
                if 'expired_access_token' in error_msg or 'expired' in error_msg.lower():
                    return jsonify({
                        'error': 'Your Dropbox access token has expired. Please generate a new token:\n\n1. Go to https://www.dropbox.com/developers/apps\n2. Select your app\n3. Go to Settings -> OAuth 2\n4. Click "Generate access token"\n5. Copy the new token and update it in your settings'
                    }), 400
                elif 'Invalid access token' in error_msg or '401' in error_msg:
