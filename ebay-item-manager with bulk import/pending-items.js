        ...
        return new Promise((resolve, reject) => {
            ...
            reader.onload = (e) => {
                try {
                    ...
                    const titleIndex = this.findColumnIndex(cleanedHeaders, ['item title', 'title', 'name', 'item', 'listing title', 'itemtitle']);
                    const imageIndex = this.findColumnIndex(cleanedHeaders, ['image', 'image url', 'url', 'photo', 'picture', 'image link', 'imageurl', 'photourl', 'imagelink', 'image_url', 'photo_url', 'image-link', 'photo-link', 'img', 'img url', 'imgurl']);
                    const priceIndex = this.findColumnIndex(cleanedHeaders, ['item subtotal', 'subtotal', 'price', 'amount', 'sale price', 'sold price', 'total', 'itemsubtotal', 'item_subtotal', 'item-subtotal']);
                    const refundIndex = this.findColumnIndex(cleanedHeaders, ['refund']);
                    ...
                    for (let i = 1; i < lines.length; i++) {
                        ...
                        const refundStr = refundIndex >= 0 ? (values[refundIndex]?.trim() || '') : '';
                ...
                reader.onload = (e) => {
                    try {
                        ...
                        jsonData.forEach(row => {
                            const title = this.findValueInRow(row, ['item title', 'title', 'name', 'item', 'listing title']);
                            const priceStr = this.findValueInRow(row, ['item subtotal', 'subtotal', 'price', 'amount', 'sale price', 'sold price', 'total']);
                            ...
                            const refundStr = this.findValueInRow(row, ['refund']);
