        ...
        return new Promise((resolve, reject) => {
            ...
            reader.onload = (e) => {
                try {
                    ...
                    const titleIndex = this.findColumnIndex(headers, ['item title', 'title', 'name', 'item', 'listing title']);
                    ...
                    const priceIndex = this.findColumnIndex(headers, ['item subtotal', 'subtotal', 'price', 'amount', 'sale price', 'sold price', 'total']);
                    const refundIndex = this.findColumnIndex(headers, ['refund']);

                    if (titleIndex === -1 || priceIndex === -1) {
                        reject(new Error('CSV must contain columns for Item Title and Item Subtotal. Image URL is optional.'));
                        return;
                    }
