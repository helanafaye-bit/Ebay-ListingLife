    ...
    async testStorageConnection() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                const error = await response.json();
                this.showStorageStatus('Connection test failed: ' + (error.error || 'Unknown error'), 'error');
            }
        } catch (error) {
            this.showStorageStatus('Cannot test connection: Backend server not available. Make sure the storage server is running.', 'error');
        }
    }
